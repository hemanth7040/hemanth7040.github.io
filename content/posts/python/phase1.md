...
```
Your Python script runs a command
         │
...
## Module 1 — `os` & `pathlib`: File and Directory Control
...
# Think of it like writing an address on a piece of paper — you haven't gone there yet.
...
subprocess.run(command, capture_output=?, text=?, check=?)
                              ...
                              │
                              └── Should Python CAPTURE the output or let it print freely?
```
...
text=False  (default)
    ...
    You cannot do normal string operations like .split(), .upper(), "main" in result.stdout
...
returncode != 0    → FAILURE  (something went wrong — the number tells you what)
...
def run_command(command: list, step_name: str, critical: bool = True):
    ...
    try:
        result = subprocess.run(
            ...
            # Example: if someone passes "git status; rm -rf /" as the command,
            # the shell would run BOTH git status AND delete your entire disk.
            #
            # With a list, each item is one argument. No shell. No injection risk.
            # ["git", "status", "; rm -rf /"] → git simply sees this as a bad argument
            # and fails safely instead of executing the dangerous part.
            ...
            # Without this, output goes directly to the terminal screen and
            # Python cannot read, log, or act on it.

            text=True,
            # Decode the output from raw bytes into a normal Python string.
            # Without text=True:
            #   result.stdout → b"On branch main\nnothing to commit\n"  (bytes object)
            #   "main" in result.stdout → ERROR: can't search bytes with a string
            # With text=True:
            #   result.stdout → "On branch main\nnothing to commit\n"   (string)
            #   "main" in result.stdout → True  (works perfectly)
            ...
            #   - Decide whether to stop or continue based on `critical` parameter
            #   - Format a useful error for the log file

            timeout=60
            # Wait a maximum of 60 seconds for the command to finish.
            # Without timeout, a hanging command (like a network call that never responds)
            # would freeze your script FOREVER.
        ...
        if result.returncode == 0:
            ...
            if result.stdout.strip():
                ...
                # We only log stdout if it actually has content (not just blank lines).
            ...
            # result.returncode tells us the specific type of failure.
            logger.error(
                f"✗ {step_name} failed "
            ...
            # If you only check stdout on failure, you will see nothing useful.
            # Always check stderr when a command fails.
        ...
        # This exception is raised when the command takes longer than `timeout` seconds.
        ...
        # This exception is raised when the command itself does not exist on the system.
...
def run_ci_pipeline():
    ...
    result = run_command(
        ...
        # We cannot deploy from a broken or dirty git state.
    )
    ...
    if result and "nothing to commit" not in result.stdout:
        ...
        # If it is NOT there, the working directory has uncommitted changes.
    ...
    run_command(
        ...
        # Deploying stale code is worse than not deploying at all.
    )

    # STEP 3 — Check kubectl cluster health (non-critical — cluster check is informational)
    result = run_command(
        command=["kubectl", "get", "nodes"],
        step_name="Kubernetes Node Health",
        critical=False
        # critical=False → if kubectl is not installed or cluster is unreachable,
        # log the error but CONTINUE the pipeline.
        # Maybe kubectl isn't available in this environment — that's OK.
    )

    if result:
        node_count = result.stdout.count("Ready")
        # result.stdout.count("Ready") counts how many times "Ready" appears in the output.
...
def write_config(config: dict, path: Path) -> None:
    ...
    with open(path, "w", encoding="utf-8") as file:
        ...
        #                    Without this, special characters (é, ñ, ₹, 中文)
        #                    may get corrupted on Windows machines.
        #
        # The `with` block guarantees the file is CLOSED when the block ends —
        # even if an error occurs mid-write. Without `with`, a crash mid-write
        # leaves the file open and possibly corrupted.
        ...
        #                    {"app":"payment","replicas":3,"env":"production"}
        #                    With indent=2, it becomes the clean multi-line format below.
        #
        # ensure_ascii=False → allow non-English characters to be written as-is.
        #                    Without this, ₹ (Indian Rupee symbol) becomes \u20b9
        #                    With this,    ₹ stays as ₹ in the file.
...
def read_config(path: Path) -> dict:
    ...
    with open(path, "r", encoding="utf-8") as file:
        ...
        #       This protects you from accidentally overwriting important files.
...
def validate_and_use_config(config: dict) -> None:
    ...
    # The default value should be a sensible fallback (1 replica is safe minimum).

    timeout = config.get("timeout_seconds", 30)
    # Not every config file will have "timeout_seconds" — maybe it was added later.
    ...
    if missing:
        ...
        # Raise an error immediately with a clear message listing exactly what is missing.
...
```
print("Deploy started")     ← what a beginner writes
...
Problems with print():
  ...
  ✗ No way to silence it in production without deleting the line

logging.info("Deploy started")   ← what a professional writes
...
Benefits of logging:
  ...
  ✓ Filter by level — show only ERROR+ in production, DEBUG+ in development
  ✓ One line change to silence all debug logs in production
```
...
DEBUG    → "I am about to open the file at /etc/app/config.json"
           ...
           Set level=DEBUG only when actively debugging a problem.

INFO     → "Deployment pipeline started for version 2.1.0"
           Normal, expected operations. The script is working as designed.
           This is your "heartbeat" — proof the script is running and progressing.
...
WARNING  → "Config key 'timeout' not found — using default 30s"
           ...
           You should investigate it tomorrow, not at 3 AM.

ERROR    → "Failed to connect to database after 3 retries"
           A real problem that needs fixing. The script may continue (with reduced
           functionality) or it may stop — but you MUST investigate this.
           Triggers alerts in production monitoring systems.
...
CRITICAL → "Disk 100% full — cannot write logs — deployment aborted"
           ...
           Wake-up-call severity — reserved for catastrophic failures only.
```
...
def create_logger(name: str, log_file: str = None) -> logging.Logger:
    ...
    # This single environment variable switch controls verbosity across ALL environments.
    logger.setLevel(log_level)
    # setLevel() sets the MINIMUM severity the logger will process.
    # Messages below this level are discarded before they even reach the handlers.
    # Think of it as a noise gate on a microphone — quiet sounds don't get through.
    ...
    # This is what you see live while the script runs.
    ...
    #
    # Example: logger.setLevel(INFO), console_handler.setLevel(DEBUG)
    # The logger discards DEBUG before it reaches any handler.
    # Setting handler to DEBUG here has no extra effect — the logger already filtered it.
    # But if logger.setLevel(DEBUG), then console shows DEBUG while
    # file_handler could be set to WARNING to keep the file clean.

    console_handler.setFormatter(formatter)
    # Attach the formatter — tell this handler how to format each line.
    # Without this, log lines have no timestamp, no level, no name — just raw text.

    logger.addHandler(console_handler)
    # addHandler() connects the handler to the logger.
    # Now: logger.info("x") → formatter formats it → console_handler prints it.
    ...
    if log_file:
        ...
        # FileHandler will crash if the directory doesn't exist.
        ...
        #
        # encoding="utf-8" → ensures special characters are written correctly.
        #                    Without this, non-ASCII characters may corrupt the file.

        file_handler.setLevel(logging.WARNING)
        # File only receives WARNING and above.
        # Reason: INFO and DEBUG create huge log files very quickly.
        # In production you want the file to contain only things that need attention.
        # The terminal still shows INFO+ for the human watching live.
    ...
    # Without this, every message can appear twice in the terminal.
...
def deploy_application(version: str, environment: str) -> bool:
    ...
    # Useful when debugging "why is the wrong config being loaded?"
    ...
    if environment == "production":
        ...
        # The deploy continues, but this line will appear in the log file
        # (because it meets the WARNING threshold for the file handler).
...
| Use `check=False` + manual `returncode` check | Full control over failure handling |
...
*Phase 1 complete. Next: Phase 2 — Cloud SDK & API Calls.*